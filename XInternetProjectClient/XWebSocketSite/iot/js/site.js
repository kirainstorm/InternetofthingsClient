	...
	if(typeof(obj.info) != "undefined")
{
	for (var i = 0; i < obj.info.length; i++) {
		...
		*/
		...
		ulll += '<div class="media" id="hyuyijyutgjnbgyui99">';
 		ulll += '<div class="media-left media-middle">';
  		ulll += '<img class="media-object" src="images/login/group.png" alt="Generic image">';
  		ulll += '</div>';
  		ulll += '<div class="media-body">';
    	ulll += '[<h4 class="media-heading">NAME:'+ obj.info[i].name + '</h4>]';
		ulll += '[<h4 class="media-heading">ID:'+ obj.info[i].uuid + '</h4>]';
		if(0 != obj.info[i].online)
		{
			ulll += '<a href="#" onclick="';
			ulll += "Fun_OpenPreview('" + obj.info[i].rtmpip + "','" + obj.info[i].rtmpuuid + "')";
		...
		ulll += '<a href="#" onclick="';
		ulll += "Fun_showModifyDeviceModal('" + obj.info[i].uuid + "','" + obj.info[i].name + "')";
		ulll += '">修改</a>';	
		
		
		ulll += '<a href="#" onclick="';
		ulll += "Fun_IOT_DeleteDevice('" + obj.info[i].uuid + "')";
		ulll += '">删除</a>';
	...
	*/
	...
	clearTimeout(_timer_refresh_device);
	$('#mainbody').LoadingOverlay("hide");
}
function Fun_IOT_RefreshDeviceError()
{
	$('#mainbody').LoadingOverlay("hide");
	alert("刷新失败");
}
function Fun_IOT_RefreshDevice()
{
	doSend(Fun_createJsonGetDevicesList());
	$('#mainbody').LoadingOverlay("show");
	_timer_refresh_device = setTimeout('Fun_IOT_RefreshDeviceError()', 3000)
}
...
//=======================================================================================
//=======================================================================================
var _timer_add_device;
function Fun_IOT_AddDeviceCallback(data)
{
	var obj = JSON.parse(data);
	if(obj.header.result == 0)
	{
		alert("添加成功");
	}
	else
	{
		alert("添加失败："+obj.header.result);
	}
	clearTimeout(_timer_add_device);
	$('#mainbody').LoadingOverlay("hide");
}
function Fun_IOT_AddDeviceError()
{
	$('#mainbody').LoadingOverlay("hide");
	alert("添加失败：超时");
}
function Fun_IOT_AddDevice()
{
	if($('#add_device_id').val() == "" || $('#add_device_name').val()=="")
	{
		alert("信息不全");
		return;
	}
	
	doSend(Fun_createJsonAddDevice($('#add_device_id').val(),$('#add_device_name').val()));
	$('#mainbody').LoadingOverlay("show");
	_timer_add_device = setTimeout('Fun_IOT_AddDeviceError()', 3000)
}
//=======================================================================================
//=======================================================================================
//=======================================================================================
//=======================================================================================
var _timer_modify_device;
function Fun_IOT_ModifyDeviceCallback(data)
{
	var obj = JSON.parse(data);
	if(obj.header.result == 0)
	{
	...
	{
		alert("修改失败："+obj.header.result);
	}
	clearTimeout(_timer_modify_device);
	$('#mainbody').LoadingOverlay("hide");
}
function Fun_IOT_ModifyDeviceError()
{
	$('#mainbody').LoadingOverlay("hide");
	alert("修改失败：超时");
}
function Fun_IOT_ModifyDevice()
{
	alert($('#modify_device_name').val());
	
	if($('#modify_device_id').val() == "" || $('#modify_device_name').val()=="")
	{
		alert("信息不全");
		return;
	}
	
	doSend(Fun_createJsonModifyDevice($('#modify_device_id').val(),$('#modify_device_name').val()));
	$('#mainbody').LoadingOverlay("show");
	_timer_modify_device = setTimeout('Fun_IOT_ModifyDeviceError()', 3000)
}
//=======================================================================================
//=======================================================================================
//=======================================================================================
//=======================================================================================
var _timer_delete_device;
function Fun_IOT_DeleteDeviceCallback(data)
{
	var obj = JSON.parse(data);
	if(obj.header.result == 0)
	{
	...
	{
		alert("删除失败："+obj.header.result);
	}
	clearTimeout(_timer_delete_device);
	$('#mainbody').LoadingOverlay("hide");
}
function Fun_IOT_DeleteDeviceError()
{
	$('#mainbody').LoadingOverlay("hide");
	alert("删除失败：超时");
}
function Fun_IOT_DeleteDevice(_uuid)
{
	doSend(Fun_createJsonDeleteDevice(_uuid));
	$('#mainbody').LoadingOverlay("show");
	_timer_delete_device = setTimeout('Fun_IOT_DeleteDeviceError()', 3000)
}
//=======================================================================================
//=======================================================================================
//=======================================================================================
