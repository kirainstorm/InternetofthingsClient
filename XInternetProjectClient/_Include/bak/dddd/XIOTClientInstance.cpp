...
void CXIotClientInstance::InitInstance(const char *pszIp, const char* pszUser, const char* pszPwd, CXSignalingChannelMessageCallback * pCallback)
{
	...
	if (CROSS_THREAD_NULL == m_hThread)
	{
		...
		s_nCenterPort = 6599;
		//
		m_pSignaling = new CXIotClientSignaling(m_pMessageCallback);
		assert(m_pSignaling);
		m_bTheadExit = FALSE;
...
void CXIotClientInstance::GetDevices(char * pdstBuffer)
{

}
emClientNetStatus CXIotClientInstance::GetSignalingStatus()
{
	return CLIENT_NET_STATUS_NONE;
}
void CXIotClientInstance::SendSignaling(const char * pszBuffer, int nLen)
{

}
CXClientStreamInterface * CXIotClientInstance::CreateXClientStreamInterface()
{
	return NULL;
}
CXClientDownloadInterface * CXIotClientInstance::CreateXClientDownloadInterface()
{
	return NULL;
}
...
void CXIotClientInstance::Woker()
{
	...
	while (FALSE == m_bTheadExit)
	{
		...
		if (m_pSignaling->IsError())
		{
			...
			//connect
			m_pSignaling->Disconnect();
			m_pSignaling->Connect(m_szIotIp, m_szUser, m_szPwd);
		}
		else
		{
			//心跳
