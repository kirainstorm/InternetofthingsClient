...
{	
	...
	else
	{
		...
		{
			//  [4/17/2015 win8]
			::PostMessage(m_hParentWndMenu, WM_MENU_BTN_CLICKED, 0, (LPARAM)NULL);
...
{
	ASSERT(m_csBitmaps[0].hBitmap != NULL);
	CSize bitmapSize;
	BITMAP bmInfo;
	VERIFY(GetObject(m_csBitmaps[0].hBitmap,sizeof(bmInfo), &bmInfo) == sizeof(bmInfo));
