...
#include "cpp_cross_display/DecDataCallback.h"
#include <time.h>
...
public:
	...
	int GetWiteMp4FileProgress();//返回0-100;
...
void CDownloadCallback::ResetFrame()
{

}

int CDownloadCallback::OnDownloadHisFileCallback(ST_SERVER_HEAD head, const char *data, int len, const void * pUser)
{
...
int CDownloadCallback::Open(CXMediaDeviceInterface *pDevInteface, DeviceInfo_T t, _XSERVER_TIME szBeginTime, _XSERVER_TIME szEndTime)
{
	m_nWiteMp4FileProgress = 0;
	m_nCureFileIndex = 0;
...
int CDownloadCallback::GetFileSecond()
{
...
int CDownloadCallback::GetDownloadFileSecond()
{
	if (!m_pDownFileBuffer)
	{
		return 0;
...
int CDownloadCallback::GetPlayFileSecond()
{
	if (!m_pDownFileBuffer)
	{
		return 0;
...
MyOnlinePlayVideoInfo * CDownloadCallback::GetNextVideoFrame()
{
	m_cs.Lock();
...
int CDownloadCallback::SeekSecond(int i)
{
	m_cs.Lock();
...
#pragma comment( lib, "c:\\output32\\mp4file.lib" )
#endif
//
int CDownloadCallback::WiteMp4File(char * filepath)
{
	int ret = -1;
	m_cs.Lock();
...
int CDownloadCallback::GetWiteMp4FileProgress()
{
