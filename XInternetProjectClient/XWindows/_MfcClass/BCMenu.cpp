...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		...
		//old vs2013不适用
		if (m_szMenuText)
			wcscpy_s(m_szMenuText, wcslen(szWideString), (wchar_t *)szWideString);
#endif
		if (m_szMenuText)
			wcscpy_s(m_szMenuText, wcslen(szWideString) + 1, (wchar_t *)szWideString);
    }
	else
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		int s1= wcslen((wchar_t *)pTp);
		//  [1/7/2015 win8]
#if 0
		wcscpy_s(szCaption, wcslen((wchar_t *)pTp),(wchar_t *)pTp); //old vs2013不适用
#endif
